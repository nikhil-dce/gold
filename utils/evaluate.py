...
def make_covariance_matrix(args, vectors, clusters, labels):
  ...
  for cluster_ix, vector_list in clusters_dict.items():
    ...
    # vector_list [hidden_dim] * Nc
    vector_tensor = torch.stack(vector_list) # [Nc, hidden_dim]
    diff = (vector_tensor - cluster) # [Nc, hidden_dim]
...
def run_inference(args, model, dataloader, exp_logger, split):
  ...
  for batch in progress_bar(dataloader, total=len(dataloader)):
    ...
    out = args.method if args.version in ['baseline', 'masker'] else 'loss'
    with torch.no_grad():
